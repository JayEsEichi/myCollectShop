...
function showProduct(folderId = null) {
    ...
    if (folderId) {
        if (isAdmin) dataSource = `/api/admin/products?sortBy=${sorting}&isAsc=${isAsc}&folderId=${folderId}`; else {
    ...
    $('#pagination').pagination({
        dataSource, locator: 'content', alias: {
            pageNumber: 'page', pageSize: 'size'
        }, totalNumberLocator: (response) => {
            return response.totalElements;
        }, pageSize: 10, showPrevious: true, showNext: true, ajax: {
            beforeSend: function () {
...
function addFolderInput() {
    $('#folders-input').append(`<input type="text" class="folderToAdd" placeholder="추가할 폴더명">
...
function addFolder() {
    ...
    $.ajax({
        type: "POST", url: `/api/folders`, contentType: "application/json", data: JSON.stringify({
            folderNames
...
function addInputForProductToFolder(productId, button) {
    $.ajax({
        type: 'GET', url: `/api/folders`, success: function (folders) {
...
`;
            ...
            $("#folder-select").on('submit', function (e) {
                e.preventDefault();
                $.ajax({
                    type: $(this).prop('method'), url: $(this).prop('action'), data: $(this).serialize()
                }).done(function () {
                    alert('성공적으로 등록되었습니다.');
                    window.location.reload();
                });
...
function execSearch() {
    ...
    $.ajax({
        type: 'GET', url: `/api/search?query=${query}`, success: function (response) {
            $('#search-result-box').empty();
