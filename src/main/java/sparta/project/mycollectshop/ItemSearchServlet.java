...
//@WebServlet(urlPatterns = "/api/search") - 컨트롤러로 대체
//servlet 방식으로 가게 된다면 api 마다 다 따로따로 클래스팔일을 새로 만들어주어야 하지만
//컨트롤러를 사용하면 이전부터 우리가 계속 사용해왔던 것처럼 컨트롤러 클래스 안에 메소드 형식으로 여러 api 들을 관리할 수 있다.
public class ItemSearchServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        headers.add("X-Naver-Client-Id", "Gh2TB3JJDi3ZjZVxwKSX");
        headers.add("X-Naver-Client-Secret", "bLPnQtAaa4");
        String body = "";
